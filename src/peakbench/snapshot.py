...
:class:`Sample` — an opaque ``id``, a ``value``, and analysis ``dims``. Dims
default to the benchmark's ``params`` (from ``@pytest.mark.parametrize``), or you
pass a ``dims_for(id)`` callback. :func:`load_long_df` stacks several files
...
def _dims(
    bm: Mapping[str, Any], dims_for: Callable[[str], Mapping[str, DimValue]] | None
) -> dict[str, DimValue]:
    """Analysis dims for one benchmark — from ``dims_for(id)`` or its parametrize params."""
    if dims_for is not None:
        return dict(dims_for(bm["fullname"]))
...
def from_pytest_benchmark(
    path: str | Path,
    *,
    dims_for: Callable[[str], Mapping[str, DimValue]] | None = None,
    metric: str = "min",
) -> tuple[str, list[Sample], str]:
    """Read timing out of a pytest-benchmark file → ``(label, samples, "s")``.

    ``metric`` picks the stat (``min`` / ``median`` / …). Dims default to each
    benchmark's parametrize ``params``; pass ``dims_for(id)`` to override.
    """
    p, benchmarks = _read_benchmarks(path)
    samples = [
        Sample(id=bm["fullname"], value=bm["stats"][metric], dims=_dims(bm, dims_for))
...
def memory_from_pytest_benchmark(
    ...
    *,
    ...
    Benchmarks lacking the key (timing-only tests) are skipped. Dims default to
    parametrize ``params``; pass ``dims_for(id)`` to override.
    """
    p, benchmarks = _read_benchmarks(path)
    samples = [
        Sample(id=bm["fullname"], value=float(bm["extra_info"][key]), dims=_dims(bm, dims_for))
        for bm in benchmarks
...
def load_samples(
    ...
    *,
    ...
    if metric == "time":
        return from_pytest_benchmark(path, dims_for=dims_for, metric=stat)
    if metric == "memory":
        return memory_from_pytest_benchmark(path, dims_for=dims_for)
...
def load_long_df(
    ...
    *,
    ...
    unit = ""
    for path in runs:
        label, samples, unit = load_samples(path, metric=metric, dims_for=dims_for, stat=stat)
        for s in samples:

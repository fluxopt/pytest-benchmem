...
def _record_memory(
    ...
    *,
    ...
    if isinstance(existing, Mapping):
        ...
        keep_bin = None if isolate else _profile_bin(getattr(benchmark, "fullname", "") or "")
...
class MemoryBenchmark:
    ...
    def __call__(self, function_to_benchmark: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        self.result = _record_memory(
            self._benchmark,
            partial(function_to_benchmark, *args, **kwargs),  # picklable when fn is top-level
...
def _install_auto_memory() -> None:
    ...
    def call(
        ...
        _record_memory(
            self,
            partial(function_to_benchmark, *a, **k),  # picklable when fn is top-level
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group.addoption(
        ...
        # argparse %-expands help text, so literal percents must be doubled or --help crashes

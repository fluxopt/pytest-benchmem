...
#: allocated), ``allocations`` (count). ``memory`` is an alias for ``peak``. Each names
#: a per-repeat series, so a ``stat`` (see :data:`STATS`) reports its distribution —
#: e.g. ``peak`` with ``--stat max`` is the worst peak across repeats.

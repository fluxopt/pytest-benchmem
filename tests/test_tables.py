...
def test_headers_and_short_names():
    blobs = {
        "mod.py::test_big": blob(4 * 1024**2, allocations=1234, total_bytes=8 * 1024**2),
        "mod.py::test_small": blob(1024, allocations=5, total_bytes=2048),
    }
...
def test_sorted_by_peak_descending():
    blobs = {
        "t::a_small": blob(1024),
        "t::b_huge": blob(9 * 1024**2),
        "t::c_mid": blob(3 * 1024**2),
    }
...
def test_single_pass_is_one_column_per_metric():
    text = _render(build_run_table({"t::x": blob(2048)}))  # repeats=1 → nothing to distribute
...
def test_multi_repeat_always_spreads_even_when_constant():
    ...
    text = _render(build_run_table({"t::x": blob([2048] * 3)}))
...
def test_unit_hoisted_into_header_and_allocations_grouped():
    blobs = {"t::x": blob(4 * 1024**2, allocations=1234567, total_bytes=2048)}
    text = _render(build_run_table(blobs))
...
def test_baseline_folds_in_columns_and_change():
    base = {"t::x": blob(10 * 1024**2)}
    head = {"t::x": blob(12 * 1024**2)}
...
def test_baseline_absent_id_shows_dash():
    base = {"t::old": blob(1024)}
    head = {"t::new": blob(2048)}  # a brand-new benchmark, not in the baseline
...
def test_no_baseline_omits_compare_columns():
    text = _render(build_run_table({"t::x": blob(2048)}))

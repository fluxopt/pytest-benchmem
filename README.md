...
`allocations` are opt-in). A fourth metric, **`rss`** — the whole-process *physical* peak
the OOM killer watches — is opt-in per test via `@pytest.mark.benchmem(isolate=True)`.
Already use `benchmark.pedantic(setup=…)` to rebuild state for
